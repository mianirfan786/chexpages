    ...
    <div>
      <Modal visible={visibleLyft} closable={false} footer={null} bodyStyle={{ height: '50%' }} wrapClassName="modalSize">
        ...
        <div className="btnComponentContainerModal">
          <Button style={{ fontSize: '15px' }} onClickButton={() => handleSubmitLyftModal(true)} title="APPLICANT" />
        </div>
        <div className="btnComponentContainerModal">
          <Button style={{ fontSize: '15px' }} onClickButton={() => handleSubmitLyftModal(false)} title="CURRENT LYFT Driver" />
        </div>
      </Modal>
    </div>
  );
